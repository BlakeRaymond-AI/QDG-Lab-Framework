...
class LabJackController(object):
    ...
    def startStream(self):
        ...
        errorCode = cDrivers.AIStreamStart(byref(idnum), demo, stateIOin, updateIO, ledOn, numChannels, byref(channels),
                                           byref(gains), byref(sampleRatePerChannel), disableCal, reserved1, readCount)
        self.ErrorHandler(errorCode)
    ...
    def readStream(self):
        ...
        errorCode = cDrivers.AIStreamRead(localID, numScans, timeout, byref(voltages), byref(stateIOout),
                                          byref(reserved), byref(ljScanBacklog), byref(overVoltage))
        self.ErrorHandler(errorCode)
        return voltages
